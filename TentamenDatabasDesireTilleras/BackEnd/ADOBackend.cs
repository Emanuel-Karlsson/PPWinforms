...
namespace HemtentaDesireDatabaserPragueParking
{
    public class ADOBackend
    {
       ...
       /// <returns></returns>
        ...
        /// </summary>
        /// <returns></returns>
        public List<Vehicle> Vehicle48h()
        {
            //List<string> listOfRegNr48h = new List<string>();
            List<Vehicle> listOfRegNr48h = new List<Vehicle>();
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        int spot = int.Parse(reader[0].ToString());
                        string regNr = (string)(reader[1]);
                        DateTime arrivalTime = DateTime.Parse(reader[2].ToString());
                        int hoursParked = int.Parse(reader[3].ToString());  
                        int vehicleType = int.Parse(reader[4].ToString());
                        var vehicle = new Vehicle(regNr, arrivalTime, null, null, vehicleType, spot, hoursParked);
                        //listOfRegNr48h.Add($"Spot: {spot} | Regnr :{regNr} | Arrival Time : {arrivalTime}  | Hours parked : {hoursParked}\n");
                        listOfRegNr48h.Add(vehicle);
                    }
                }
                catch (Exception)
                {
